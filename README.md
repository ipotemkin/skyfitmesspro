...
Б. Локальный запуск приложения с **прокси-сервером** для хранения refteshToken в httpOnly cookie:

```sh
npm run proxy
```
В этом режиме делается продакшен-сборка приложения и запусается proxy-server в режиме перезагрузки, чтобы можно было интерактивно менять код прокси сервера (src/serverProxy.js)
