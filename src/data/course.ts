// все курсы
// для настройки фронта, не ответ API!
export const mockCourses = {
  1: {
    id: 1,
    name: 'Йога',
    coverUrl: 'assets/images/card-violet.png',
    descritption: `Йога - это философия здорового образа жизни. Тот, кто занимается йогой, становится здоровее и выносливее, после занятий чувствует прилив сил, а также с новой силой может ощутить вкус к жизни.    
    Благодаря комплексному воздействию упражнений происходит проработка всех групп мышц, тренировка суставов, улучшается циркуляция крови. Кроме того, упражнения дарят отличное настроение, заряжают бодростью и помогают противостоять стрессам.`,
    sutableFor: [
      'Давно хотели попробовать йогу, но не решались начать.',
      'Хотите укрепить позвоночник, избавиться от болей в спине и суставах.',
      'Ищете активность, полезную для тела и души.',
    ],
    lines: [
      'Йога для новичков',
      'Классическая йога',
      'Йогатерапия',
      'Кундалини-йога',
      'Хатха-йога',
      'Аштанга-йога',
    ],
    workouts: [
      {
        id: 1,
        name: 'Утренняя практика / Йога на каждый день / 1 день / Алексей Казубский',
        video_url: 'https://youtu.be/oqe98Dxivns',
        excercises: [
          {
            id: 1,
            name: 'Приветствие солнца',
            retriesCount: 10,
          },
        ],
      },
      {
        id: 2,
        name: 'Красота и здоровье / Йога на каждый день / 2 день / Алексей Казубский',
        video_url: 'https://youtu.be/v-xTLFDhoD0',
        excercises: [
          {
            id: 1,
            name: 'Наклон вперед',
            retriesCount: 10,
          },
          {
            id: 2,
            name: 'Наклон назад',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Поднятие ног, согнутых в коленях',
            retriesCount: 5,
          },
        ],
      },
      {
        id: 3,
        name: 'Асаны стоя / Йога на каждый день / 3 день / Алексей Казубский',
        video_url: 'https://youtu.be/WxFz-4YsiEE',
        excercises: [
          {
            id: 1,
            name: 'Наклон к правой ноге',
            retriesCount: 10,
          },
          {
            id: 2,
            name: 'Наклон к левой ноге',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Наклон к согнутой правой ноге',
            retriesCount: 10,
          },
          {
            id: 4,
            name: 'Наклон к согнутой левой ноге',
            retriesCount: 10,
          },
          {
            id: 5,
            name: 'Асана стоя',
            retriesCount: 5,
          },
        ],
      },
      {
        id: 4,
        name: 'Растягиваем мышцы бедра / Йога на каждый день / 4 день / Алексей Казубский',
        video_url: 'https://youtu.be/09uGkAEQuZI',
        excercises: [
          {
            id: 1,
            name: 'Сесть на пятки с носками от себя',
            retriesCount: 5,
          },
          {
            id: 2,
            name: 'Сесть на пятки с носками на себя',
            retriesCount: 5,
          },
          {
            id: 3,
            name: 'Отпустить колено на пол из позы лотоса',
            retriesCount: 10,
          },
          {
            id: 4,
            name: 'Отпустить колено на пол из позы лотоса с соединенными стопами',
            retriesCount: 10,
          },
        ],
      },
      {
        id: 5,
        name: 'Гибкость спины / Йога на каждый день / 5 день / Алексей Казубский',
        video_url: 'https://youtu.be/MIvcMapie_A',
        excercises: [
          {
            id: 1,
            name: 'Округляем грудную клетку при выдохе',
            retriesCount: 10,
          },
          {
            id: 2,
            name: 'Тянем левую руку в правую сторону',
            retriesCount: 20,
          },
          {
            id: 3,
            name: 'Тянем правую руку в левую сторону',
            retriesCount: 20,
          },
        ],
      },
    ],
  },
  2: {
    id: 2,
    name: 'Стретчинг',
    coverUrl: 'assets/images/card-blue.png',
    descritption: `Стретчинг (или stretching) – это система упражнений, целью которых является разогрев и растяжка мышц и связок. При этом стретчинг – не просто комплекс упражнений для разминки перед тренировкой, а самостоятельное направление фитнеса, которое может использоваться как в комплексе с другими направлениями, так и самостоятельно.
    Стретчинг в домашних условиях может использоваться для многих целей: 
    - Выступает в качестве гимнастики в период восстановления после травм;
    - Входит в состав программы для похудения;
    - Помогает развить гибкость и пластичность, при правильном подходе вы сядете на шпагат через несколько недель;
    - Это эффективный способ расслабиться после трудного дня.`,
    sutableFor: [
      'Хотите подтянуть тело, смоделировать мышечный корсет',
      'Улучшить осанку и координацию',
      'Улучшить кровообращение и обмен веществ',
    ],
    lines: ['статический', 'динамический', 'пассивный', 'активный'],
    workouts: [
      {
        id: 1,
        name: 'Основы стретчинга',
        video_url: 'https://youtu.be/Ewm-Bfg5ncg',
        excercises: [
          {
            id: 1,
            name: 'Правильное дыхание',
            retriesCount: 20,
          },
          {
            id: 2,
            name: 'Наклон вниз, правая рука тянется вверх',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Наклон вниз, левая рука тянется вверх',
            retriesCount: 10,
          },
          {
            id: 4,
            name: 'Перенос веса с ноги на ногу в положении сидя',
            retriesCount: 20,
          },
        ],
      },
      {
        id: 2,
        name: 'Разогрев мышц',
        video_url: 'https://youtu.be/IISxKE3LGSE',
        excercises: [
          {
            id: 1,
            name: 'Перекрестные взмахи руками',
            retriesCount: 15,
          },
          {
            id: 2,
            name: '«Собака мордой вверх»',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Скручивание лежа',
            retriesCount: 5,
          },
        ],
      },
      {
        id: 3,
        name: 'Разогрев мышц 2.0',
        video_url: 'https://youtu.be/HKB89EIJ2J0',
        excercises: [
          {
            id: 1,
            name: 'Поперечная складка вперед',
            retriesCount: 15,
          },
          {
            id: 2,
            name: '«Поза ребенка»',
            retriesCount: 5,
          },
          {
            id: 3,
            name: 'Растяжка трицепсов и плеч',
            retriesCount: 20,
          },
        ],
      },
    ],
  },
  3: {
    id: 3,
    name: 'Бодифлекс',
    coverUrl: 'assets/images/card-lightblue.png',
    descritption: `BodyFlex – система, в которой особым образом сочетаются физические упражнения на укрепление и растяжку мышц, и дыхательная гимнастика. Такое сочетание приводит к ряду положительных эффектов, которые практически невозможно достичь с помощью других направлений фитнеса. Одна из самых интересных особенностей данной системы заключается в том, что это эффективный фитнес дома.
    Весь смысл бодифлекса сводится к правильному дыханию, благодаря которому упражнения на растяжку и укрепление мышц приобретают свою эффективность. При выдыхании воздуха и задержке дыхания организм на короткое время испытывает кислородное голодание, в крови накапливается углекислый газ, что приводит к запуску некоторых физиологических процессов, типичных для ситуации «бей или беги». То есть, организм приходит в состояние повышенной готовности и способен выполнять быстрые и энергичные действия.
    `,
    sutableFor: [
      'Хотите укрепить легкие и снизить вероятность заболеваний дыхательной системы',
      'Улучшить пищеварение',
      'Ускорить метаболизм',
    ],
    lines: ['базовый', 'продвинутый'],
    workouts: [
      {
        id: 1,
        name: 'Техника дыхания',
        video_url: 'https://youtu.be/HruMFn74NII',
        excercises: [
          {
            id: 1,
            name: 'Отведение рук назад',
            retriesCount: 20,
          },
          {
            id: 2,
            name: 'Боковая растяжка',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Простой пресс',
            retriesCount: 20,
          },
        ],
      },
      {
        id: 2,
        name: 'Тренировка мышц бедер',
        video_url: 'https://youtu.be/zxaoes0JQ5g',
        excercises: [
          {
            id: 1,
            name: 'Ножницы горизонтальные',
            retriesCount: 10,
          },
          {
            id: 2,
            name: 'Ножницы вертикальные',
            retriesCount: 10,
          },
          {
            id: 3,
            name: 'Шлюпка',
            retriesCount: 15,
          },
        ],
      },
      {
        id: 3,
        name: 'Тренировка мышц ягодиц',
        video_url: 'https://youtu.be/JTCaG3Y9xlM',
        excercises: [
          {
            id: 1,
            name: 'Крендель',
            retriesCount: 15,
          },
          {
            id: 2,
            name: 'Собачка',
            retriesCount: 20,
          },
          {
            id: 3,
            name: 'Кошка',
            retriesCount: 15,
          },
        ],
      },
    ],
  },
  4: {
    id: 4,
    name: 'Танцевальный фитнес',
    coverUrl: 'assets/images/card-orange.png',
    descritption: `Фитнес и танцы – что между ними общего? А то, что они могут великолепно сочетаться и оказывать просто восхитительный эффект на ваше тело! Объединение хореографии и аэробики – это необычно и интересно, именно так родился танцевальный фитнес, которым вы теперь можете заниматься дома. Достичь отличной формы и при этом получить удовольствие вам поможет видео для похудения, которое мы представляем на этой странице – делайте упражнения, танцуйте и чувствуйте, как ваше тело меняется в лучшую сторону!`,
    sutableFor: [
      'Хотите укрепить мышцы, но тренировки в тренажерном зале для вас скучные и однообразные',
      'Хотите сбросить лишний вес, но нет желания мучать себя диетами',
      'Любите танцы и хотите совместить приятное с полезным.',
    ],
    lines: ['Зумба', 'Dance-mix', 'Caribbean-mix'],
    workouts: [
      {
        id: 1,
        name: 'Урок 1. Основы',
        video_url: 'https://youtu.be/x6_8ZEYXPak',
        excercises: [],
      },
      {
        id: 2,
        name: 'Урок 2. Основные движения',
        video_url: 'https://youtu.be/UjVtcMD4On4',
        excercises: [],
      },
      {
        id: 3,
        name: 'Урок 3. Новые движения',
        video_url: 'https://youtu.be/gvL27uHhKz4',
        excercises: [],
      },
      {
        id: 4,
        name: 'Урок 4. Продвинутые движения',
        video_url: 'https://youtu.be/3RPauxe4SeE',
        excercises: [],
      },
      {
        id: 5,
        name: 'Урок 5. Мастер-класс',
        video_url: 'https://youtu.be/_78_OGLyy9Y',
        excercises: [],
      },
    ],
  },
  5: {
    id: 5,
    name: 'Степ-аэробика',
    coverUrl: 'assets/images/card-lightgreen.png',
    descritption: `Степ-аэробика – направление фитнеса, основанное на наборе аэробных упражнений, выполняемых с помощью специальной ступеньки (степ-платформы). Данное направление было разработано в 1989 году Джиной Миллер: травмировав колено, она обратилась к различным методикам восстановления сустава, но наибольший эффект показали простые упражнения в виде шагов на молочном ящике. Впоследствии эти упражнения Миллер положила в основу целого комплекса, который и стал называться степ-аэробикой.
    Занятия степ-аэробикой состоят из комплексов в виде различного сочетания шагов, выполняемые под музыку с довольно высоким темпом. Каждое занятие длится от 30 до 50 минут в достаточно высоком темпе без остановок на отдых – для передышки используются переходы на простые шаги и наиболее простые упражнения. Такой режим тренировок приводит к эффективному сжиганию калорий, укреплению суставов и общему улучшению состояния здоровья, что и стало основой высокой популярности нового направления.`,
    sutableFor: [
      'Хотите укрепить дыхательную и сердечно-сосудистой системы',
      'Быстрый сбросить лишние килограммы',
      'Улучшить настроение, повысить жизненный тонус',
    ],
    lines: ['Для начинающих', 'Для похудения', 'Для детей'],
    workouts: [
      {
        id: 1,
        name: 'Урок 1. Основы',
        video_url: 'https://youtu.be/oK2mdodtPY4',
        excercises: [],
      },
      {
        id: 2,
        name: 'Урок 2. Основные движения',
        video_url: 'https://youtu.be/gJPs7b8SpVw',
        excercises: [],
      },
      {
        id: 3,
        name: 'Урок 3. Новые движения',
        video_url: 'https://youtu.be/PGAQfXIAY7k',
        excercises: [],
      },
      {
        id: 4,
        name: 'Урок 4. Продвинутые движения',
        video_url: 'https://youtu.be/zuBLSarZuyk',
        excercises: [],
      },
    ],
  },
};

export const mockCoursesList = [
  {
    id: 1,
    name: 'Йога',
    coverUrl: 'assets/images/card-violet.png',
  },
  {
    id: 2,
    name: 'Стретчинг',
    coverUrl: 'assets/images/card-blue.png',
  },
  {
    id: 3,
    name: 'Бодифлекс',
    coverUrl: 'assets/images/card-lightblue.png',
  },
  {
    id: 4,
    name: 'Танцевальный фитнес',
    coverUrl: 'assets/images/card-orange.png',
  },
  {
    id: 5,
    name: 'Степ-аэробика',
    coverUrl: 'assets/images/card-lightgreen.png',
  },
];

// информация по тренировке пользователя по выбранному курсу
export const mockUserWorkout = {
  id: 2,
  name: 'Красота и здоровье / Йога на каждый день / 2 день / Алексей Казубский',
  video_url: 'https://youtu.be/v-xTLFDhoD0',
  excercises: [
    {
      id: 1,
      name: 'Наклон вперед',
      retriesCount: 10,
      userProgress: 5,
    },
    {
      id: 2,
      name: 'Наклон назад',
      retriesCount: 10,
      userProgress: 3,
    },
    {
      id: 3,
      name: 'Поднятие ног, согнутых в коленях',
      retriesCount: 5,
      userProgress: 2,
    },
  ],
};

// все тренировки пользователя по выбранному курсу
export const moskUserWorkouts = [
  {
    id: 1,
    name: 'Утренняя практика / Йога на каждый день / 1 день / Алексей Казубский',
    done: true,
  },
  {
    id: 2,
    name: 'Красота и здоровье / Йога на каждый день / 2 день / Алексей Казубский',
    done: true,
  },
  {
    id: 3,
    name: 'Асаны стоя / Йога на каждый день / 3 день / Алексей Казубский',
    done: false,
  },
  {
    id: 4,
    name: 'Растягиваем мышцы бедра / Йога на каждый день / 4 день / Алексей Казубский',
    done: false,
  },
  {
    id: 5,
    name: 'Гибкость спины / Йога на каждый день / 5 день / Алексей Казубский',
    done: false,
  },
];
